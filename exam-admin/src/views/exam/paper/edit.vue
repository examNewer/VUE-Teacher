<template>
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="100px" v-loading="formLoading" :rules="rules">
      <el-form-item label="类型：" prop="ceId" required>
        <el-select v-model="form.ceId" placeholder="类型"  >
          <el-option v-for="item in subjectFilter"  :key="item.ceId"  :value="item.ceId" :label="item.ceName"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="试卷编写人：" v-if="form.teacher!==null" prop="teacher" required>
        <el-input v-model="form.realname"/>
      </el-form-item>
      <el-form-item label="试卷编写人：" prop="tid" required v-show="false"  >
        <el-input v-model="form.tid" placeholder="试卷编写人ID"/>
      </el-form-item>
      <el-form-item label="创建时间：" prop="createTime"  v-if="form.createTime!==null" required>
        <el-date-picker v-model="form.createTime" value-format="yyyy-MM-dd HH:mm" type="datetime" placeholder="试卷创建时间"></el-date-picker>
      </el-form-item>
      ...
      <el-form-item label="试卷名称："  prop="examCode" v-if="form.examCode!==null" required>
        <el-input v-model="form.examCode"/>
      </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        tid: [
          { required: true, message: '编写人ID', trigger: 'blur' }
        ],
        ceId: [
        ...
        createTime: [
          { required: true, message: '请选择试卷开始时间', trigger: 'change' }
        ],
        name: [
  ...
  created () {
    ...
    if (id && parseInt(id) !== 0) {
      ...
      examPaperApi.select(id).then(re => {
        ...
        _this.form.realname = re.response.teacher.realname
